...
Partial Class flighttim
    ...
    Sub display()
        ...
        't = "select flightid from flighttime"
    ...
    Protected Sub Button2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        If DropDownList2.SelectedItem.Text = DropDownList3.SelectedItem.Text Then
            Response.Redirect("flexist.aspx")
        Else
            If rs1.State = 1 Then rs1.Close()
            rs1.Open("select * from flighttime", cn)
            If Not rs1.EOF Then
                While Not rs1.EOF
                    r1 = rs1(0).Value
                    r2 = rs1(1).Value
        ...
        If flag = 0 Then
            If rs1.State = 1 Then rs1.Close()
            sql = "insert into flighttime values('" & DropDownList1.SelectedItem.Text & "','" & DropDownList2.SelectedItem.Text & "','" & DropDownList3.SelectedItem.Text & "','" & TextBox1.Text & "','" & TextBox2.Text & "')"
            rs1.Open(sql, cn)
            Response.Redirect("flreg.aspx")
        Else
            Response.Redirect("flexist1.aspx")
        End If
